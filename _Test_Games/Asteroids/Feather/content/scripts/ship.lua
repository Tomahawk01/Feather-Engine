...
function Ship:AddLives(lives)
	self.numLives = F_clamp(self.numLives + lives, 0, 3)
...
function Ship:AddShieldHealth( amount )
	...
	if not self.shieldEnabled and self.shieldHealth > 0 then 
		self.shieldEnabled = true 
...
function Ship:UpgradeWeapon()
	...
	self.currentWeapon = F_clamp(self.currentWeapon + 1, 0, 3)
...
function Ship:TakeDamage(amount)
	...
	if self.shieldEnabled then
		self.shieldHealth = F_clamp(self.shieldHealth - amount, 0, 100)
		...
		self.shipHealth = F_clamp(self.shipHealth - amount, 0, 100)
...
function Ship:UpdateExplosion()
	if self.explode and self.deathTimer:elapsed_ms() > 1000 then 
		local ship = Entity(self.entityID)
		local animation = ship:getComponent(Animation)
