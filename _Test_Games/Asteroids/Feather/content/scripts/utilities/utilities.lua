...
WINDOW_WIDTH = F_WindowWidth()
WINDOW_HEIGHT = F_WindowHeight()
...
function LoadEntity(def)
	...
	if def.components.sprite then
		...
		sprite:generateUVs()
		sprite.isHidden = def.components.sprite.isHidden or false
	end
...
function FlashSprite(sprite, timer, time)
	if time <= 0 then 
		F_warn("time value in FlashSprite, cannot be zero or less")
		return
	end
    ...
    if (math.floor(elapsed / time) % 2) == 0 then
        sprite.isHidden = false  -- Visible
    else
        sprite.isHidden = true   -- Hidden
    end
end
...
function FlashColor(sprite, timer, time, color1, color2)
	if time <= 0 then 
		F_warn("time value in FlashColor, cannot be zero or less")
		return
	end
    -- Start the timer if it isn't already running
    if not timer:is_running() then
