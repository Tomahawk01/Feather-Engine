...
GameOverState = F_Class("GameOverState")
...
function GameOverState:Init(params)
	...
	self.gameOverLabel:addComponent(
		Transform(
			vec2(
				(F_WindowWidth() * 0.5) - 256, 
				(F_WindowHeight() * 0.5) - 32
			), 
			vec2(4, 4), 0.0
		)
	)
	...
	asteroidTransform.position = vec2(
		(F_WindowWidth() * 0.5) - (asteroidSprite.width * 0.5),
...
function GameOverState:UpdateScreenAnim()
	...
	if asteroidTransform.scale.x < 15.0 then 
		...
		asteroidTransform.position = vec2(
			(F_WindowWidth() * 0.5) - (asteroidSprite.width * 0.5 * asteroidTransform.scale.x),
			(F_WindowHeight() * 0.5) - (asteroidSprite.height * 0.5 * asteroidTransform.scale.y)
		)
		
		asteroidTransform.rotation = asteroidTransform.rotation + 15
		
	elseif not self.isBrokenScreen then
		-- Break screen effect and reveal "Game Over" text
		self.isBrokenScreen = true 
