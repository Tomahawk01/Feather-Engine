...
-- @brief Initializes the HUD by setting up lives indicators, score display, health/shield bars, and event handlers for ship updates.
function Hud:Init()
...
function Hud:UpdateHealth(event)
	local ship = gShipHandler:GetShipByID(event.data.event.shipID)
	if not ship then 
		F_warn("Failed to update health. Ship [%d] is invalid", event.data.event.shipID)
		return 
	end
...
function Hud:UpdateShield(event)
	local ship = gShipHandler:GetShipByID(event.data.event.shipID)
	if not ship then 
		F_warn("Failed to update Shield. Ship [%d] is invalid", event.data.event.shipID)
		return 
	end
