...
namespace Feather {
	...
	void EditorStyleToolDisplay::DrawSizePicker()
	{
		...
		if (ImGui::InputFloat("Tab Rounding", &style.TabRounding, 1.0f, 1.0f, "%0.1f"))
			style.TabRounding = std::clamp(style.TabRounding, 0.0f, 10.0f);

		if (ImGui::InputFloat("Tab Border Size", &style.TabBorderSize, 1.0f, 1.0f, "%0.1f"))
			style.TabBorderSize = std::clamp(style.TabBorderSize, 0.0f, 10.0f);

		if (ImGui::InputFloat("Frame Border Size", &style.FrameBorderSize, 0.1f, 0.1f, "%0.1f"))
			style.FrameBorderSize = std::clamp(style.FrameBorderSize, 0.0f, 1.0f);

		if (ImGui::InputFloat("Frame Padding Width", &style.FramePadding.x, 1.0f, 1.0f, "%0.1f"))
			style.FramePadding.x = std::clamp(style.FramePadding.x, 0.0f, 10.0f);

		if (ImGui::InputFloat("Frame Padding Height", &style.FramePadding.y, 1.0f, 1.0f, "%0.1f"))
			style.FramePadding.y = std::clamp(style.FramePadding.y, 0.0f, 10.0f);

		if (ImGui::InputFloat("Frame Rounding", &style.FrameRounding, 1.0f, 1.0f, "%0.1f"))
			style.FrameRounding = std::clamp(style.FrameRounding, 0.0f, 10.0f);

		if (ImGui::InputFloat("Scrollbar Size", &style.ScrollbarSize, 1.0f, 1.0f, "%0.1f"))
			style.ScrollbarSize = std::clamp(style.ScrollbarSize, 1.0f, 10.0f);

		if (ImGui::InputFloat("Scrollbar Rounding", &style.ScrollbarRounding, 1.0f, 1.0f, "%0.1f"))
			style.ScrollbarRounding = std::clamp(style.ScrollbarRounding, 0.0f, 10.0f);

		if (ImGui::InputFloat("Window Border Size", &style.WindowBorderSize, 0.1f, 0.1f, "%0.1f"))
			style.WindowBorderSize = std::clamp(style.ScrollbarRounding, 0.0f, 1.0f);

		if (ImGui::InputFloat("Window Padding Width", &style.WindowPadding.x, 1.0f, 1.0f, "%0.1f"))
			style.WindowPadding.x = std::clamp(style.WindowPadding.x, 0.0f, 10.0f);

		if (ImGui::InputFloat("Window Padding Height", &style.WindowPadding.y, 1.0f, 1.0f, "%0.1f"))
			style.WindowPadding.y = std::clamp(style.WindowPadding.y, 0.0f, 10.0f);

		if (ImGui::InputFloat("Window Rounding", &style.WindowRounding, 1.0f, 1.0f, "%0.1f"))
			style.WindowRounding = std::clamp(style.WindowPadding.y, 0.0f, 10.0f);

		if (ImGui::InputFloat("Window Title Align X pos", &style.WindowTitleAlign.x, 0.1f, 0.1f, "%0.1f"))
			style.WindowTitleAlign.x = std::clamp(style.WindowTitleAlign.x, 0.0f, 1.0f);

		if (ImGui::InputFloat("Item Spacing X pos", &style.ItemSpacing.x, 1.0f, 1.0f, "%0.1f"))
			style.ItemSpacing.x = std::clamp(style.ItemSpacing.x, 0.0f, 10.0f);

		if (ImGui::InputFloat("Item Inner Spacing X", &style.ItemInnerSpacing.x, 0.1f, 0.1f, "%0.1f"))
			style.ItemInnerSpacing.x = std::clamp(style.ItemInnerSpacing.x, 0.0f, 10.0f);

		if (ImGui::InputFloat("Item Inner Spacing Y", &style.ItemInnerSpacing.y, 0.1f, 0.1f, "%0.1f"))
			style.ItemInnerSpacing.y = std::clamp(style.ItemInnerSpacing.y, 0.0f, 10.0f);

		if (ImGui::InputFloat("Button Text Align X pos", &style.ButtonTextAlign.x, 0.1f, 0.1f, "%0.1f"))
			style.ButtonTextAlign.x = std::clamp(style.ButtonTextAlign.x, 0.0f, 1.0f);

		if (ImGui::InputFloat("Button Text Align Y pos", &style.ButtonTextAlign.y, 0.1f, 0.1f, "%0.1f"))
			style.ButtonTextAlign.y = std::clamp(style.ButtonTextAlign.y, 0.0f, 1.0f);
		ImGui::PopItemWidth();
